...
const keygen = {
	...
	next256() {
		let hex = keygen.hash[keygen.step256] + keygen.hash[keygen.step256+1]
		let rnd = parseInt(hex, 16) 
		keygen.step256 = keygen.step256 + 2 
		if(keygen.step256 >= keygen.hash.length) {
			keygen.step256 = 0
		}
		return rnd
	},
	...
	getKeyParams(id) {
		let key = keygen.sha256(id)
		let params = []
		for(let i=0; i<key.length; i=i+2) {
			let k = key.charAt(i)+key.charAt(i+1)
			params.push({ hex: k, dec: parseInt(k, 16) })
		}
		keygen.step16 = 0
		keygen.step256 = 0
		keygen.step4095 = 0
	...
	sha256(ascii) {
		...
		var hash = keygen.sha256.h = keygen.sha256.h || [];
		// Round constants: first 32 bits of the fractional parts of the cube roots of the first 64 primes
		var k = keygen.sha256.k = keygen.sha256.k || [];
